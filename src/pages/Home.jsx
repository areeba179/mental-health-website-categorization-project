...
function Home() {
 ...
 const handleSearch = async () => {
  ...
  /* =========================
     🔥 IMPROVED REPO NAME SEARCH
  ========================== */
  else if (searchType === "name") {

    try {

      // STRONG SEARCH STRATEGY (FIX)
      const queries = [
        `${query} mental health`,
        `${query} therapy`,
        `${query} psychology`,
      ...
      if (unique.length === 0) {
        return showError("No repositories found for this name.");
      }

      // OPEN TOP RESULTS (LIMIT TO 5)
      unique.slice(0, 5).forEach(repo => {
